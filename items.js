Inventory = function (){
    ...
    self.refreshRender = function(){
        ...
        for(var i = 0; i < self.helmets.length; i++){
            ...
            let use = "Item.list['" + item.id + "'].event()";
            let sell = "Item.list['" + item.id + "'].sell()";
            str3 += "<div class = 'potionList' id = '"+item.name+"1'><div class = 'potion-info-block'><img src = './img/"+item.id+".png' class = 'img-items'><div class = 'info-items'>"+item.name+"<br> "+item.describtion+"</div><button id ='btn"+item.id+"' class = 'btn-use-item' onclick = \"" + use + "\" >Use</button></div></div>";
...
Item = function(id, name, type, describtion, event, unUse, sell, buy){
    var self = {
        id:id,
        name:name,
        type:type,
...
Item ("potion", "Potion", "potions","Heals to full", function(){
...
Item ("potionSpeed", "PotionSpeed", "potions", "Sets speed to 12", function(){
    player.speed = 12;
...
Item("Skull","Skull", "helmets","+1 to strength", function(){
    if(player.isHatOn){
...
Item("FrostHood","FrostHood", "helmets","+1 to dexterity", function(){
    if(player.isHatOn){
        Item.list[player.hat].unUse()
        document.getElementById('btn'+ player.hat).disabled = false;
    }
...
Item("HatOfGreatWizardry","HatOfGreatWizardry", "helmets","+1 to wisdom", function(){
    if(player.isHatOn){
        Item.list[player.hat].unUse()
        document.getElementById('btn'+ player.hat).disabled = false;
    }
...
Item("RubyDiadem","RubyDiadem", "helmets","+1 to intellect", function(){
    if(player.isHatOn){
        Item.list[player.hat].unUse()
        document.getElementById('btn'+ player.hat).disabled = false;
    }
